...
void StartButtonTask(void *argument)
{
  /* USER CODE BEGIN 5 */
	...
	vPrintUART("button task started-blocking on semaphore\r\n");
  /* Infinite loop */
  for(;;)
  {
    xQueueSemaphoreTake(xButtonSemaphoreHandle, portMAX_DELAY);
    pressCount++;
    HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
    sniprintf(buf, sizeof(buf), "button task : button pressed count = %lu LED = %s\r\n", pressCount, (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_5)==GPIO_PIN_SET)? "ON" : "OFF");
    vPrintUART(buf);
